        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6">
          <div className="py-12 md:py-16">
            ...
            <p className="mx-auto mt-6 max-w-2xl text-center text-sm text-ink/70 md:text-base">
              Choose a direction below. Each service is built around your guest experience — timing, staffing,
              plating, and venue flow.
            </p>
          </div>
        </div>
        ...
        <div className="sticky top-20 z-30 border-y border-line bg-cream/95 backdrop-blur md:top-24">
          <div className="mx-auto max-w-7xl px-4 sm:px-6">
            <div className="no-scrollbar flex items-center gap-6 overflow-x-auto py-4 text-[11px] uppercase tracking-[0.3em] text-ink/70">
              {services.map((s) => (
                <a key={s.id} href={`#${s.id}`} className="whitespace-nowrap hover:text-ink">
                  {s.name}
                </a>
              ))}
            </div>
            ...
            {services.map((service, idx) => {
              ...
              return (
                <FadeIn key={service.id}>
                  <article id={service.id} className="grid gap-8 lg:grid-cols-12 lg:items-center">
                    <div
                      className={`overflow-hidden bg-ink lg:col-span-7 ${flip ? "lg:order-2" : ""}`}
                    >
                      <img
                        src={service.image}
                        alt={service.name}
